...
int main() {
    ...
    {
        for(int i = 0; i<=3; i++){
            rx = spi_readwrite(message[i]);     // Send binary 0000 0000 1010 1010
            printf("Recieved: %u\n",rx);    // Display the value returned by the FPGA
            wait_us(1000000);    
        }

        rx = spi_readwrite(512+144);
        printf("Recieved: %u\n",rx);    // Display the value returned by the FPGA
        wait_us(1000000);

        
        rx = spi_readwrite(BPM[0]);
        printf("Recieved: %u\n",rx);    // Display the value returned by the FPGA
        wait_us(1000000);
        rx = spi_readwrite(BPM[1]);
        printf("Recieved: %u\n",rx);    // Display the value returned by the FPGA
        wait_us(1000000);
        ...
        // rx = spi_readwrite(512+64);     // Send binary 0000 0000 1010 1010
        // printf("Recieved: %u\n",rx);    // Display the value returned by the FPGA
        // wait_us(1000000);               // 
        // rx = spi_readwrite(512+84);     // Send binary 0000 0000 0101 0101
        // printf("Recieved: %u\n",rx);    // Display the value returned by the FPGA
        // wait_us(1000000);               //
        // rx = spi_readwrite(512+144);
        // printf("Recieved: %u\n",rx);    // Display the value returned by the FPGA
